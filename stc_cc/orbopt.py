...
def matrix_operation_numpy(A, func):
    """Apply a scalar function to the eigenvalues of a symmetric matrix (numpy).
    numpy analogue of la.matrix_operation (which is torch-only)."""
...
# ===========================================================================
# GHF / spinor support. A spin-free 1-electron AO operator M is block-diagonal
# diag(M, M) in the 2*nao spinor-AO space (verified: GHF's overlap, dipole, and
# atomic projectors are all diag(M, M) with the first nao rows the alpha-spin AO
# block and the last nao the beta-spin block). Hence for a spinor coefficient
# matrix C (2*nao rows), C^T M C = C_a^T M C_a + C_b^T M C_b -- transform each
# spin block with the SAME spatial M and ADD. Localization classes that build a
# molecular-integral MO operator (Boys/PM/BoysOVX) take spinor=True and route
# their C^T M C through these helpers; DFAll/DFAux consume an already-MO DF
# tensor and need no change.
# ===========================================================================
def _spin_blocks(C, spinor):
    """Rows of coefficient matrix C split into its two spatial-AO spin blocks
    (alpha = top nao, beta = bottom nao) when spinor, else the single block (C,)."""
    if not spinor:
        return (C, )
    nao = C.shape[0] // 2
    return (C[:nao], C[nao:])
...
def _transform_spin(M, Cl_blocks, Cr_blocks):
    """sum_s Cl_s^T M Cr_s for one AO operator M over matched spin blocks. A
    single-block (non-spinor) side -- e.g. the spin-free aux basis -- pairs with
    every block of a spinor side (the cross orbital/aux terms of BoysOVX)."""
    n = max(len(Cl_blocks), len(Cr_blocks))
    pick = lambda blocks, i: blocks[i] if len(blocks) > 1 else blocks[0]
    out = None
    for i in range(n):
        term = pick(Cl_blocks, i).T @ M @ pick(Cr_blocks, i)
...
class GeneralOrbLoc:
    ...
    def transform(self, A: torch.Tensor) -> torch.Tensor:
        if self.unitary:
            return self.cayley(A)
        else:
...
class OrbLoc(GeneralOrbLoc):
    ...
    def cost_function_numpy(self, u: np.ndarray = None) -> np.ndarray:
        if u is None:
            u = np.eye(self.n)
...
class MultiOrbLoc(GeneralOrbLoc):
    ...
    def cost_function_from_A(self, *As: torch.Tensor) -> torch.Tensor:
        us = [self.transform(A) for A in As]
    ...
    def run(self, solver_builder, tol, masks=None, verbose=True):
        ...
        us_opt = [self.transform(A) for A in As_opt]
...
class Boys(OrbLoc):
    def __init__(self, mol, C_init, spinor=False):
        self.n = C_init.shape[1]
        r = mol.intor_symmetric('int1e_r', comp=3)
        r2 = mol.intor_symmetric('int1e_r2')
        blk = _spin_blocks(C_init, spinor)
        r_init = np.stack([_transform_spin(x, blk, blk) for x in r])
        r2_init = _transform_spin(r2, blk, blk)
...
class PM(OrbLoc):
    def __init__(self, mol, C_init, spinor=False, **kwargs):
        import pyscf.lo
        self.n = C_init.shape[1]
        # atomic populations are spin-additive; build proj per spin block (each a
        # cheap AO->MO transform, so no nao-sized object even when nmo << nao) and
        # ADD. atomic_pops is bilinear in C, so this equals C^T P_A C on the spinor.
        proj = sum(pyscf.lo.pipek.atomic_pops(mol, Cs, **kwargs)
                   for Cs in _spin_blocks(C_init, spinor))
        self.proj = torch.from_numpy(proj)  # [natoms x n x n] per PySCF convention
        self.norm = torch.linalg.norm(self.proj)**2

    def cost_function(self, u: torch.Tensor) -> torch.Tensor:
        # proj_diag[x,a] = sum_{i,j} proj[x,i,j] * u[i,a] * u[j,a]
        proj_diag = torch.einsum('xij,ia,ja->xa', self.proj, u, u)
        #return -torch.sum(proj_diag**2)
        return self.norm - torch.sum(proj_diag**2)


class BoysOVX(MultiOrbLoc):
    def __init__(self, mol, auxmol, Cocc, Cvir, spinor=False):
        import pyscf.gto
        self.mol = mol
        self.auxmol = auxmol
        ...
        Caux = matrix_operation_numpy(Saux, lambda x: 1.0 / np.sqrt(x))
        self.Caux = Caux
        ...
        # spin-block sum on the ORBITAL side (spinor); the aux basis is spin-free
        # (single block, never doubled). Non-spinor -> single block -> the plain
        # CMO^T x CMO / CMO^T x Caux / Caux^T x Caux transforms as before.
        cmo_blk = _spin_blocks(CMO, spinor)
        aux_blk = (Caux, )

        mol_r = mol.intor_symmetric('int1e_r', comp=3)
        mol_r2 = mol.intor_symmetric('int1e_r2')
        mol_r = np.stack([_transform_spin(x, cmo_blk, cmo_blk) for x in mol_r])
        mol_r2 = _transform_spin(mol_r2, cmo_blk, cmo_blk)

        cross_r = pyscf.gto.mole.intor_cross('int1e_r', mol, auxmol, comp=3)
        cross_r2 = pyscf.gto.mole.intor_cross('int1e_r2', mol, auxmol)
        cross_r = np.stack([_transform_spin(x, cmo_blk, aux_blk) for x in cross_r])
        cross_r2 = _transform_spin(cross_r2, cmo_blk, aux_blk)

        auxmol_r = auxmol.intor_symmetric('int1e_r', comp=3)
        auxmol_r2 = auxmol.intor_symmetric('int1e_r2')
        auxmol_r = np.stack([_transform_spin(x, aux_blk, aux_blk) for x in auxmol_r])
        auxmol_r2 = _transform_spin(auxmol_r2, aux_blk, aux_blk)
    ...
    def post_process(self, Uocc, Uvir, Uaux):
        ...
        j2c_sqrt = matrix_operation_numpy(j2c, lambda x: np.sqrt(x))
        j2c_local = Caux_local.T @ j2c @ Caux_local
        j2c_local_invsqrt = matrix_operation_numpy(j2c_local, lambda x: 1.0 / np.sqrt(x))
...
class FockNorm(OrbLoc):
    def __init__(self, F_ao, C_init):
        self.n = C_init.shape[1]
        F_init = C_init.T @ F_ao @ C_init
        self.F_init = torch.from_numpy(F_init)
        self.norm = torch.linalg.norm(self.F_init)**2

    def cost_function(self, u: torch.Tensor) -> torch.Tensor:
        #Fdiag = torch.einsum('pq,pi,qi->i', self.F_init, u, u)
        F_full = u.T @ self.F_init @ u
        return torch.sum(F_full**2) - torch.sum(torch.diag(F_full)**2)
        #return self.norm - torch.sum(Fdiag**2)
