...
def get_alias_numba(prob_in, prob, alias, this_sign, alias_sign):
    ...
    # Partition first.  The previous search-loop implementation assumed that a
    # large entry always exists.  For exactly uniform rows all probabilities are
    # one, so the large search ran out of bounds and wrote garbage aliases.
    indices_by_sign = np.empty((N, ), dtype=np.int32)
    nsmall = 0
    nlarge = N
    for i in range(N):
        if prob[i] < 1.0:
            indices_by_sign[nsmall] = i
            nsmall += 1
        else:
            nlarge -= 1
            indices_by_sign[nlarge] = i

    idx_small = nsmall - 1
    idx_large = nsmall
    while idx_small >= 0 and idx_large < N:
        i_small = indices_by_sign[idx_small]
        i_large = indices_by_sign[idx_large]
        alias[i_small] = i_large
        prob[i_large] = prob[i_large] - (1.0 - prob[i_small])
        if prob[i_large] < 1.0:
            indices_by_sign[idx_small] = i_large
            idx_large += 1
        else:
            idx_small -= 1
...
@nb.njit(inline='never', **njit_kwargs)
def get_alias_numba_old(prob_in, prob, alias, this_sign, alias_sign):
    N = len(prob_in)
    #this_sign[:] = prob_in < 0
